...
const WorkingHoursRow = ({ day, isOpen, handleToggle }) => {
    ...
    const [dayTimePicker, setDayTimePicker] = useState([]);

    const handleAddClick = () => {
        const newBlock = { id: dayTimePicker.length + 1, isOpen: true };
        setDayTimePicker([...dayTimePicker, newBlock])
    };
    const handleDeleteClick = (id) => {
        const updatedBlocks = dayTimePicker.filter(item => item.id !== id);
        setDayTimePicker(updatedBlocks);
    }
        ...
        <>
            <div className="working-row">
                ...
                <Box display="flex" alignItems="center" gap={2} disabled={!isOpen}>
                    ...
                    <TimePickerComponent initialValue="2024-02-01T17:59" disabled={!isOpen} />
                </Box>
                <button type="button" className="add-period-button" onClick={handleAddClick} disabled={!isOpen}>
                    +
                </button>
            </div>


            {dayTimePicker.map((block) => (
                <div className='working-row'>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginLeft: '220px' }}>
                        <Box display="flex" alignItems="center" gap={2} key={block.id}>
                            <TimePickerComponent initialValue="2024-02-01T09:00" disabled={!isOpen} />
                            <Typography>to</Typography>
                            <TimePickerComponent initialValue="2024-02-01T17:59" disabled={!isOpen} />
                            <button type="button" className='modal-delete' title="Delete" onClick={() => handleDeleteClick(block.id)} disabled={!isOpen}>
