...
const Availability = () => {
  ...
  const handleEventClick = (event) => {
    updateState({ eventAnchor: event.currentTarget });
  };
  ...
  const handleCalendarClose = () => {
    updateState({ eventAnchor: null, timeZoneAnchor: null });
  };
    ...
    <div className="calendar_availability">
      ...
      >
        <DialogTitle sx={{ ...styles.selectdateTitle }} >
          New schedule
          <IconButton
            onClick={() => updateState({ newScheduleOpen: false })}
      ...
      <Box >
          ...
          <>
            <div className="scedule_container">
              ...
              <div className="scedule_btn_container">
                ...
                <Button variant="text" className="createscedule_btn" onClick={() => updateState({ newScheduleOpen: true })}>+ Create schedule</Button>
              </div>
                ...
                <div className="header_container">
                  <div style={{ display: "flex", gap: "40px", alignItems: "center", padding: '15px' }}>
                    ...
                    <div>
                      ...
                      >
                        {state.selected ? "1 Event Type" : "No Event Type"} <ExpandMoreIcon fontSize="small" />
                      </div>
                      <EventTypeMenu
                        anchorEl={state.eventAnchor}
                        open={Boolean(state.eventAnchor)}
                        onClose={handleCalendarClose}
                        selected={state.selected}
                        // setSelected={setSelected}
                        setSelected={(value) => updateState({ selected: value })}
                      />
                    </div>
                  ...
                  <ToggleButtonGroup className="scedule_calendar_toggle"
                    value={state.view}
                    exclusive
                    onChange={(e, newView) => newView && updateState({ view: newView })}
        ...
        {state.tabIndex === 1 && (
          <div className="holiday_calendar">
            <Dialog
              open={state.holidayOpen}
              onClose={() => updateState({ holidayOpen: false })}
              maxWidth="xs"
              fullWidth
              PaperProps={{
                sx: { maxWidth: "360px" }
            ...
            <div>
              ...
              <div className="country_used_holiday_container">
                <div className="country_holiday_content">
                  ...
                  <div><b>Other</b><span onClick={() => updateState({ holidayOpen: true })}><a>Change</a></span></div>
                </div>
