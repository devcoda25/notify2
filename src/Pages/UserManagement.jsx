...
import { Table, TableBody, TableCell, TableHead, TablePagination, TableRow, Checkbox, Autocomplete, TextField, Chip } from '@mui/material';
import boy from '../Component/Assets/img/boy.png';
import { Modal, ModalBody } from 'react-bootstrap';
const initialTableData = [
    { id: 1, img: boy, userName: "Thameem Hameed", status: 'Online', email: 'thameem@heptotechnologies.org', role: 'ADMINISTRATOR', teams: 'All Teams' },
    { id: 2, img: boy, userName: "EV ZONE", status: 'Offline', email: 'info@evzoneafrica.com', role: 'ADMINISTRATOR', teams: 'All Teams' },
    { id: 3, img: boy, userName: "juliet_1", status: 'Offline', email: 'juliet_1@evzoneafrica.com', role: 'TEMPLATE MANAGER; OPERATOR;', teams: 'Ev_zone_everyone' }
]
...
const AddUserModal = ({ show, onClose, initialContent }) => {
                ...
                <div className='userrule_main_content'>
                    ...
                    <Modal.Body className='userrule_modal_bodycontent'>
                        <div >
                            ...
                            />
                            <Checkbox /><span className='edit__text__label'>Email Confirmed</span>
                            <div className='edit__text__label'>Phone Number</div>
                            <input type="text" placeholder="Phone Number" className='edit__text__input'
                            />
                            <Checkbox /><span className='edit__text__label'>Phone Confirmed</span>
                        ...
                        <button className='btn btn-success'>Save</button></div>
                    </Modal.Body>
                ...
                <div className='userrule_main_content'>
                    ...
                    <Modal.Body className='userrule_modal_bodycontent'>
                        <div >
                            ...
                            <Checkbox /><span className='edit__text__label'>Default Team</span>
                        </div>
                        <div className='savebtn'>
                        <button className='btn btn-success'>Add Team</button></div>
                    </Modal.Body>
...
const UserManagement = () => {
    ...
    const [usermanagementData, setUsermanagementData] = useState(initialTableData);
    const [teamsData, setTeamsData] = useState(initialTeamData)
    const [searchUsermanagement, setSearchUsermanagement] = useState('');
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [modalType, setModalType] = useState(null); //user or team
    ...
    const filterUsermanagement = usermanagementData.filter(row =>
        row.userName.toLowerCase().includes(searchUsermanagement.toLowerCase())
    );
    //teams
    ...
    return (
        <>
            {isModalOpen && (
                <>
                    {modalType === 'user' && <AddUserModal show={isModalOpen} onClose={handleCloseModal} />}
                    {modalType === 'teams' && <AddTeamModal show={isModalOpen} onClose={handleCloseModal} />}
                </>
            )}
                    ...
                    <div className="usermanagement_header">
                        ...
                        <div className='right_container'>
                            <div className='action_wrap'>
                                ...
                                <button className='btn btn-success adduserbtn' onClick={() => handleOpenModal(selectedOption)}> {selectedOption === 'user' ? 'Add User' : 'Add Team'}</button>
                            </div>
                        </div>
                    </div>
                                ...
                                <Table className='usermanagement_table'>
                                    <TableHead className='usermanagement_head'>
                                        <TableRow className='usermanagement__row'>
                                            ...
                                            <TableCell className='usermanagement__cell lastcell' style={{ width: '110px' }}>Actions</TableCell>
                                        </TableRow>
                                    </TableHead>
                                    <TableBody className='usermanagement__table__body'>
                                        {filterUsermanagement.map((data, index) => (
                                            <TableRow key={index} className='body_row'>
                                                <TableCell className='body__cell body_first_cell'><Checkbox sx={{ marginRight: 1 }} /></TableCell>
                                                <TableCell className='body__cell' ><div className='flex_item'>
                                                    <img src={data.img} className='agent_image' /><div className='info_container'><p>{data.userName}</p></div></div> </TableCell>
                                                ...
                                                <TableCell>
                                                    <button className='cell__arrow' ><svg className='arrowsvg' stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"></path></svg></button>
                                                    <button className='cell__moreicon'  ><svg className='moreiconsvg' stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 16 16" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M3 9.5a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3zm5 0a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3zm5 0a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3z"></path></svg></button>
                                                </TableCell>
                                            </TableRow>
                                        ))}
                                    ...
                                    <Table className='usermanagement_table'>
                                        <TableHead className='usermanagement_head'>
                                            <TableRow className='usermanagement__row'>
                                                ...
                                                <TableCell className='usermanagement__cell' style={{ width: '150px' }}>Default Name</TableCell>
